...
import { getEnabledSharePlatforms, SHARE_DIALOG_CONFIG } from "@/lib/sharePlatforms";
...
export default function ArticleShareActions({ title, summary, url, className }: Props) {
    ...
    <Dialog>
      ...
      <DialogContent className="max-w-md! border border-border bg-background p-5 dark:border-white/10">
        <DialogHeader>
          ...
          <DialogDescription>
            Share <span className="font-medium text-foreground">{title}</span> on your preferred
            platform.
          </DialogDescription>
        </DialogHeader>
