...
function CategoryCard({
	...
	return (
		<div className="rounded-2xl border border-border/70 bg-muted/35 p-4 dark:border-white/10 dark:bg-white/[0.03]">
			<div className="flex items-start justify-between gap-3">
				<div className="flex min-w-0 gap-3">
					<div className="mt-0.5 inline-flex size-9 shrink-0 items-center justify-center rounded-full border border-border/70 bg-background/80 text-foreground dark:border-white/10 dark:bg-black dark:text-white">
						<Icon className="size-4" strokeWidth={2} />
					</div>
					<div className="min-w-0">
						<div className="flex items-center gap-2">
							<h3 className="text-sm font-medium text-foreground">{title}</h3>
							<span className="rounded-full border border-border/70 bg-muted/40 px-2 py-0.5 text-[11px] font-medium uppercase tracking-[0.12em] text-foreground/80 dark:border-white/10 dark:bg-black dark:text-white/70">
								{valueLabel}
							</span>
						</div>
						<p className="mt-1 text-sm leading-6 text-foreground/70 dark:text-white/65">{description}</p>
					</div>
				</div>
				<Switch
					checked={isActive}
		...
		<>
			{bannerVisible ? (
				<div className="fixed bottom-24 right-4 z-20 w-[min(28rem,calc(100vw-6.5rem))] lg:bottom-8 lg:right-4">
					<div className="rounded-lg border border-border bg-background/70 p-4 text-foreground shadow-lg shadow-black/5 backdrop-blur-2xl saturate-150 dark:border-white/20 dark:bg-black/50 dark:text-white dark:shadow-black/20">
						<div className="flex items-start justify-between gap-3">
							<div className="space-y-2">
								<div className="inline-flex items-center gap-2 rounded-full border border-border px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.14em] text-foreground/70 dark:border-white/10 dark:text-white/70">
									<Cookie className="size-3.5" strokeWidth={2} />
									Privacy Choices
								</div>
								<h2 className="text-base font-medium">Choose how this site uses cookies</h2>
								<p className="text-sm leading-6 text-foreground/65 dark:text-white/65">
							...
							<Button
								...
								className="border border-border bg-background/75 hover:bg-muted dark:border-white/10 dark:bg-black/75 dark:hover:bg-black"
			...
			>
				...
				<DialogContent className="max-w-xl! border border-border bg-background p-0 dark:border-white/10">
					...
					<div className="flex flex-col gap-3 border-t border-border bg-foreground/[0.02] p-4 dark:border-white/10 dark:bg-white/[0.02] sm:flex-row sm:items-center sm:justify-between">
						<a
							href="/legal/privacy"
							className="text-sm text-foreground/65 underline underline-offset-4 hover:text-foreground/85 dark:hover:text-white"
						>
